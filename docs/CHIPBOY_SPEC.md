...
*Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §25:*
bars leave the model entirely, beats per bar included — a phrase's own length (1–64
steps) replaces it, and the host's bars stay a ruler as before.
...
*Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §7 and
§26:* measured against a real LSDj ROM, the chip's own envelope generator never runs —
every NRx2 goes out with its low nibble forced to **8**, and **Hardware envelope**'s own
rate and direction are stepped by the driver itself, off the pitch clock, at a measured
rate per step (`docs/LSDJ_PARITY.md` §7). Every step that produces, like any other level
change, is a zombie-mode write and never a retrigger — this section's "one write to NRx2
on note-on and then the chip runs it" is not what the hardware is measured doing. This
mode is now called **Chip**; the instrument also gains **Shaped** (§27): Attack, Peak,
Decay, Sustain and Release, each with its own curve.
...
*Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §27:* these
three rows are now the **Chip** side of an envelope **mode**; the alternative, **Shaped**,
replaces them with Attack, Peak, Decay, Sustain and Release in ticks, each with its own
curve (linear, exponential or logarithmic), rendered as one level a tick. Wave and kit
instruments take the four NR32 levels either way.
...
A **hex display** preference is available for people coming from LSDj. It
is a display format, not an accuracy switch, so it does not conflict with **C8**.

*Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §34:* built
as two complete views rather than a per-field option, resolving D7 below — **Decimal**
shows every letter's `x, y` (nibbles, a signed byte, or its own small range per letter)
and **Hex** shows the one byte a command occupies, LSDj's own encoding (`P` two's
complement, `T`'s BPM byte, a nibble pair). So the packed byte from the previous
paragraph *does* appear, in Hex mode, because that byte is exactly what a playback ROM
would carry.
...
- **Wave editor.** 32 × 16 grid, frame strip along the bottom, shape generators,
  interpolate-between-frames. *Revised 2026-09-09: see
  [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §33:* gains a **synth** — a source
  and a chain of shapers, morphed over a run of frames — generated by core code
  (`bank::Synth`, `bank::synthesize`) rather than drawn by hand alone.
...
- **Phrases.** A tracker: per channel a note, instrument, table and two command columns,
  ...
  *Revised 2026-09-08 (third): see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md)
  ...
  steps.
  *Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §25:*
  bars leave the model — a phrase's own length (1–64 steps) replaces "sixteen steps to a
  phrase, phrases chained by bar," and the chain highlights each channel's own playing
  row where the STP override sat; the embedding song format has since moved on to **7**.
  *Revised 2026-09-09: see [`docs/COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §30:* a
  note takes Shift+arrows, a drag, or double-click typing with auto-correction, and every
  slot field — this tab's INS/TBL included — shares one convention: click types,
  right-click lists, double-click opens the item's own tab.
...
| **D7** | ~~Hex display option for LSDj users?~~ **Resolved 2026-09-09.** | Built as two full views of a command's byte, Decimal and Hex, one per letter shape (`docs/COMMANDS_AND_TEMPO.md` §34) — display only, so **C8** is unaffected. |
