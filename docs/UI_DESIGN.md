...
| D-UI-22 | How does an LSDj save come in? | ***Import .sav…* in the Tracker head's FILE group, then a dialog** ([`COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §54, §56): the chooser takes a `.sav` and `.lsdprj` / `.lsdsng` project files, several at once; the songs the save holds and each project, each with a checkbox — the working song first, named after the file it was loaded from with *(working copy)* — its format and the LSDj version it will be read by; a *Version* dropdown (*Auto* by each song's format, or one version for all, the assumed ones marked); what ROM the folder held. *Import* opens a tab per song, activates the last, and puts the notes on what could not be carried over exactly in a message box; the status line counts them |
| D-UI-23 | What helps a frame be drawn by hand? | **A centre line on each axis**, over the trace so the Bars view cannot bury them: between levels 7 and 8 -- the DAC's own zero, where a frame with no offset sits -- and between samples 15 and 16. The Bars view's bar grows **up** from the floor, filling from the sample's own row down, so a loud sample is a tall bar even though [`COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §107 puts level 0 at the top |
| D-UI-24 | Can a frame be drawn without the mouse? | **Yes, with the arrows.** The sample last clicked is the selection, outlined -- the whole column faintly, its own cell in full, brighter while the grid holds the keyboard. Left and right walk the samples, up and down move the selected one's level, up being a *smaller* number because §107 draws level 0 at the top. An edit goes through the same path a click does, so undo covers it. Nothing else is taken, so Tab still leaves the grid |
| D-UI-25 | Which way up does a channel scope draw? | **Level 0 at the top, 15 at the bottom** (§106): a scope traces the channel's *digital* levels, and the DMG's DACs invert, so drawn this way round it is the shape that comes out of the plugin. The baseline for a channel the mix has silenced moves to level 7.5, the DAC's zero, which is where the analog trace already puts a DAC-off sample. The master scope is unchanged -- it plots the rendered output, which already carries the DAC |
| D-UI-26 | Where does a pulse instrument's finetune go? | **A *Finetune* stepper on the Instrument tab**, beside PU2 transpose ([`COMMANDS_AND_TEMPO.md`](COMMANDS_AND_TEMPO.md) §112): 0-255, in 256ths of a semitone, **down** when the instrument plays on PU1 and **up** on PU2, which is what LSDj's byte 11 does and what makes a pair of pulses beat. A cell's `F` replaces it for the note in progress |
| D-UI-27 | Where does a kit note's second sample go? | **The VEL column**, on rows whose instrument is a kit ([`plan-kit-pairs.md`](plan-kit-pairs.md)): `01` up to the kit's sample count names another of its samples to sum with the note's, blank plays one. The column counts to the kit's length instead of 127 there, and its help line names the sample it is on. Velocity means nothing else on a kit — `velToVolume` only reaches Pulse and Noise — and the keyswitch velocity mode skips a kit so the column means one thing at a time |
...
| D-UI-33 | What happens when a save is imported without its ROM? | **The dialog says so in the warning colour and offers *Choose ROM…***: a `.gb` of any name, or a `.zip` holding one; among several the one whose version reads the songs' format wins, then the newest, and a ROM whose version cannot be read still gives its kits. A save carries no samples -- a kit instrument names a bank of the ROM -- so without one the kit instruments are skipped with a note, and a custom kit needs the ROM it was patched into (the .lsdsng and the .sav hold only the kit's number) |
