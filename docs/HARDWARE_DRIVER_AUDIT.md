...
| ~~**The length counter disabling a channel**~~ — **settled by §138** | This row said a zombie write to a channel a length counter has stopped is computed as if it were running, and that fixing it meant modelling the 256 Hz frame step for one silent case. Both halves were wrong. It is not silent: it is four noise hits a bar in `READROOM`. And nothing needs to know *whether* the counter has expired — measured on the ROM, an **`E` always triggers when the counter is enabled**, which is a bit of the instrument. The driver does that now (§138); a level change that is not an `E` still zombie-writes, so §26 stands everywhere else. |
