...
game or from LSDj. Each is a **format 5** `.cbsong`, which means it carries its own bank:
...
| `meter-study` | 20 · 126 BPM · 3/4, 12 steps | **Bar lengths, at one tempo.** A waltz at twelve six-tick steps to the bar; bar 8 is a **step override of 14** — 84 ticks, seven eighths — and bars 15–16 are **20 steps**, five beats, a 5/4 stretch. The tempo never moves: only the bars do, and every channel takes the same ruler (§11). | `flute`, `chord` (pulse), `bass` (triangle), `reed-bass` (a harmonic wave at half level), `kick` `snare` `hat` |
