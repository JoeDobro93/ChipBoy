...
_spec = importlib.util.spec_from_file_location("d", "/root/lsdj/dump.py")
...
HOST = '/root/lsdj/kits.sav'
HOST_IDX = 0                               # CASTSHDW, format 22
HOST_CHAIN = [0x18, 0x17, 0x15, 0x16]      # its song row 0, per channel
HOST_PHRASE = [0x12, 0x02, 0x3B, 0x42]     # the first phrase of each of those
...
class Probe:
    def __init__(self):
        self.sav = bytearray(open(HOST, 'rb').read())
        self.s = bytearray(d.decompress(self.sav, HOST_IDX))
        assert self.s[0x7FFF] == 22, "host song is not format 22"
