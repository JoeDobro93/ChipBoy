...
namespace {
/// The head: two rows of tools over the line that says what the song is and
/// what the last file did. 112 px of head and 400 px of lane -- a 48 px
/// grid header and sixteen 22 px rows -- are the 512 the editor pane has at
/// the window's minimum height (UI_DESIGN sections 2 and 7).
constexpr int kToolRow = 26, kToolGap = 6, kHeadHeight = 112, kChainGap = 12;
      ...
      export_("Export .gb" + ellipsis())
{
    ...
    for (auto* c : std::initializer_list<Component*>{ &play_, &stop_, &loop_, &playLed_, &playText_, &pos_, &rec_, &stepsLabel_, &steps_,
                                                     &startLabel_, &songStart_, &beatsLabel_, &beats_, &saveSong_, &loadSong_, &export_,
                                                     &report_, &scroll_, &chain_ }) addAndMakeVisible(c);
    ...
    saveSong_.setTooltip("Write the song " + String(CharPointer_UTF8("\xe2\x80\x94")) + " chains, phrases, grooves, steps, arms and its tempo " + String(CharPointer_UTF8("\xe2\x80\x94"))
                         + " to a .cbsong file. The bank stays where it is; the file remembers its name and the names of the slots the song uses.");
    saveSong_.onClick = [this] { saveSong(); };
    loadSong_.setTooltip("Read a .cbsong file. It replaces the song; with the bank it was written with it sounds the same, and this line says where this bank differs.");
...
void TrackerPanel::refreshViews()
{
    ...
    syncSongTime();
    if (reported_.runs.empty()) showSongSummary();
}
...
void TrackerPanel::showReport(const RichText& text)
{
    reported_ = text;
    report_.setText(text);
    message(text.toString());
}

void TrackerPanel::showSongSummary()
{
    const auto s = processor.song();
    const int bars = s ? std::max(1, s->bars()) : 0;
    int used = 0, overrides = 0;
    if (s) {
        for (const auto& ph : s->phrases) if (ph.used) ++used;
        for (const auto v : s->barSteps) if (v != 0) ++overrides;
    }
    RichText r;
    if (songFile_ != File()) r.bold(songFile_.getFileNameWithoutExtension()).plain(middot());
    r.bold(String(bars) + (bars == 1 ? " bar" : " bars"));
    r.plain(middot() + String(s ? s->steps() : 16) + " steps a bar"
            + middot() + String(used) + (used == 1 ? " phrase" : " phrases"));
    if (overrides > 0) r.plain(middot()).bold(String(overrides) + (overrides == 1 ? " bar" : " bars")).plain(" with a step count of its own");
    reported_ = {};
    report_.setText(r);
}
...
{
    const String name = File::createLegalFileName(songFile_ != File() ? songFile_.getFileNameWithoutExtension() : processor.bankName() + " song");
    ...
    chooser_->launchAsync(kSaveFlags, [safe = Component::SafePointer<TrackerPanel>(this)](const FileChooser& fc) {
        ...
        RichText r;
        if (safe->processor.saveSongFile(file)) {
            safe->songFile_ = file;
            r.plain("Saved ").bold(file.getFileName()).plain(" to " + file.getParentDirectory().getFullPathName());
        }
...
void TrackerPanel::loadSong()
{
    ...
    chooser_->launchAsync(kOpenFlags, [safe = Component::SafePointer<TrackerPanel>(this)](const FileChooser& fc) {
        ...
        SongReport rep;
        RichText r;
        if (!safe->processor.loadSongFile(file, rep)) {
        ...
        safe->bar_ = 0;
        safe->refreshViews();
        r.plain("Loaded ").bold(file.getFileNameWithoutExtension());
        r.plain(dash() + "written with bank ").bold(rep.bankName.isEmpty() ? String("(unnamed)") : rep.bankName);
...
void TrackerPanel::syncTransport(bool force)
{
    ...
    if (force || owns != owns_) {
        ...
        // (section 16), so the song's timeline is live either way.
...
void TrackerPanel::resized()
{
    ...
    // the transport, what it is doing, the arm, and what a step is worth
    place(top, play_, 62, 24);
    top.removeFromLeft(4);
    place(top, stop_, 62, 24);
    top.removeFromLeft(4);
    place(top, loop_, 52, 24);
    top.removeFromLeft(14);
    place(top, playLed_, 8, 8);
    top.removeFromLeft(6);
    place(top, playText_, 54, kToolRow);
    place(top, pos_, 62, kToolRow);
    top.removeFromLeft(12);
    place(top, rec_, 62, 24);
    top.removeFromLeft(16);
    label(top, stepsLabel_);
    place(top, steps_, steps_.preferredWidth(), Stepper::kHeight);

    // the song's own timeline (docs/COMMANDS_AND_TEMPO.md section 4), then
    // the song's file and, one day, its ROM
    ...
    place(bottom, saveSong_, 104, 24);
    bottom.removeFromLeft(6);
    place(bottom, loadSong_, 104, 24);
    bottom.removeFromLeft(16);
    place(bottom, export_, 96, 24);

    report_.setBounds(head);
